...
for i_episode in range(training_episodes):
  ...
  while done==False:
    if player==1:
      marker1 = env.p1_marker
      marker2 = env.p2_marker
      p1network = q_network_p1
      p2network = q_network_p2
      p1avg = p1_episode_history
      p2avg = p2_episode_history
    else:
      marker1 = env.p2_marker
      marker2 = env.p1_marker
      p1network = q_network_p2
      p2network = q_network_p1
      p1avg = p2_episode_history
      p2avg = p1_episode_history
    state_p1 = state
    action_p1 = p1network.eGreedyAction(state[np.newaxis,:])
    next_state, reward_p1, done = env.step(action_p1, marker1)
    # play opponent move
    if done == False:
      state_p2 = np.array(next_state)
      action_p2 = p2network.eGreedyAction(state[np.newaxis,:])
      next_state, reward_p2, done = env.step(action_p2, marker2)
    ...
    p1network.storeExperience(state_p1, action_p1, reward_p1, next_state, done)
    p1network.updateModel()
    p2network.storeExperience(state_p2, action_p2, reward_p2, next_state, done)
    p2network.updateModel()
    ...
    if done == True:
      if reward_p1 == 10:
        ...
        p1_lost += 1
        p2_won += 1
      p1avg.append(reward_p1)
      p2avg.append(reward_p2)
      break
